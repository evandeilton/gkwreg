...
#' @examples
#' \dontrun{
#' # Generate sample data from Kumaraswamy distribution
#' set.seed(123)
#' data <- rkw(n = 200, alpha = 2.5, beta = 1.8)
#'
#' # Fit the Kumaraswamy distribution
#' fit <- gkwfit(data, family = "kw")
#'
#' # Compute and display goodness-of-fit statistics
#' gof <- gkwgof(fit)
#'
#' # Get more detailed output with simulated p-values
#' gof <- gkwgof(fit, simulate_p_values = TRUE, n_bootstrap = 500)
#'
#' # Detailed summary with verbose output
#' gof <- gkwgof(fit, verbose = TRUE)
#'
#' # Get just the statistics without plots or printed summary
#' gof_stats <- gkwgof(fit, plot = FALSE, print_summary = FALSE)
#'
#' # Compare multiple models
#' fit_beta <- gkwfit(data, family = "beta")
#' fit_gkw <- gkwfit(data, family = "gkw")
#'
#' gof_kw <- gkwgof(fit, print_summary = FALSE)
...
#' print(ic_comparison)
#'
#' # Generate a comparison plot of multiple fitted distributions
#' compare_gof_plot(list(KW = gof_kw, Beta = gof_beta, GKW = gof_gkw))
#' }
#'
#' @seealso \code{\link{gkwfit}}, \code{\link{summary.gkwfit}}, \code{\link{print.gkwgof}},
#'          \code{\link{plot.gkwgof}}, \code{\link{compare_gof_plot}}
#'
#' @references
...
#'
#' @export
compare_gof_plot <- function(gof_list, criteria = "all", plot = TRUE, plot_type = "all", ...) {
