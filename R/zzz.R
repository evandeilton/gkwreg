...
#' #' Package environment for TMB data
#' #' @noRd
#' .gkwreg_tmb_env <- new.env(parent = emptyenv())
#'
...
#' }
#'
#' #' Determine cache directories based on platform
#' #' @noRd
#' get_cache_dirs <- function(pkg_name, is_windows = FALSE) {
#'   if (is_windows) {
#'     # Windows: prefer C:/R, fallback to AppData or rappdirs
#'     cache_dir <- file.path("C:/R", paste0(pkg_name, "_tmb_cache"))
#'     if (!dir.exists(cache_dir) && !dir.create(cache_dir, recursive = TRUE, showWarnings = FALSE)) {
#'       if (requireNamespace("rappdirs", quietly = TRUE)) {
#'         cache_dir <- file.path(rappdirs::user_cache_dir(pkg_name), "tmb_cache")
#'       } else {
#'         cache_dir <- file.path(Sys.getenv("LOCALAPPDATA"), paste0(pkg_name, "_tmb_cache"))
#'         if (!dir.exists(dirname(cache_dir))) {
#'           cache_dir <- file.path(Sys.getenv("APPDATA"), paste0(pkg_name, "_tmb_cache"))
#'         }
#'       }
#'     }
#'   } else {
#'     # Unix/macOS: use rappdirs or ~/.R/<pkg>_tmb_cache
#'     if (requireNamespace("rappdirs", quietly = TRUE)) {
#'       cache_dir <- file.path(rappdirs::user_cache_dir(pkg_name), "tmb_cache")
#'     } else {
#'       cache_dir <- file.path(path.expand("~"), ".R", paste0(pkg_name, "_tmb_cache"))
...
#'   r_version <- paste0("R", getRversion()[1, 1:2])
#'   pkg_version <- tryCatch(as.character(packageVersion(pkg_name)), error = function(e) "dev")
#'   version_dir <- file.path(cache_dir, r_version, pkg_version)
#'   if (!dir.exists(version_dir)) dir.create(version_dir, recursive = TRUE, showWarnings = FALSE)
#'   list(cache_dir = cache_dir, version_dir = version_dir)
#' }
#'
#' #' Locate the TMB C++ source file in standard package paths
#' #' @noRd
#' locate_cpp_file <- function(dll_name, pkg_name) {
#'   paths <- c(
#'     system.file("tmb", paste0(dll_name, ".cpp"), package = pkg_name),
#'     system.file("src/tmb", paste0(dll_name, ".cpp"), package = pkg_name),
#'     system.file("src", paste0(dll_name, ".cpp"), package = pkg_name),
#'     file.path("inst/tmb", paste0(dll_name, ".cpp")),
#'     file.path("src/tmb", paste0(dll_name, ".cpp")),
#'     file.path("src", paste0(dll_name, ".cpp"))
#'   )
...
#'     }
#'   }
#'   stop("Could not find TMB source file '", dll_name, ".cpp' in standard locations.")
#' }
#'
#' #' Try loading a cached DLL; decide if compilation is needed
#' #' @noRd
#' check_and_load_cached <- function(dll_name, cached_path, cpp_file, force_recompile, verbose) {
#'   need_compile <- force_recompile || !file.exists(cached_path)
#'   if (!need_compile) {
#'     # Compare modification times
#'     if (file.info(cpp_file)$mtime > file.info(cached_path)$mtime) {
#'       log_msg(verbose, "Source is newer than cached DLL; will recompile.")
#'       need_compile <- TRUE
#'     } else {
#'       load_try <- try(
#'         {
#'           norm_path <- normalizePath(cached_path, mustWork = TRUE)
#'           if (!any(sapply(getLoadedDLLs(), function(x) x$path == norm_path))) dyn.load(norm_path)
#'           .gkwreg_tmb_env[[paste0("TMB_", dll_name, "_PATH")]] <- norm_path
#'           TRUE
#'         },
#'         silent = !verbose
#'       )
#'       if (!inherits(load_try, "try-error")) {
#'         log_msg(verbose, "Loaded cached TMB model: ", cached_path)
#'         dll_info <- list(
#'           path = cached_path,
#'           normalized_path = normalizePath(cached_path, mustWork = FALSE),
#'           name = tools::file_path_sans_ext(basename(cached_path)),
#'           compiled = FALSE
#'         )
#'         .gkwreg_tmb_env[[paste0("TMB_", dll_name, "_INFO")]] <- dll_info
#'         return(list(dll_info = dll_info, need_compile = FALSE))
#'       } else {
#'         log_msg(verbose, "Failed to load cached DLL; recompiling.")
#'         try(dyn.unload(cached_path), silent = TRUE)
#'         need_compile <- TRUE
#'       }
#'     }
#'   }
#'   list(dll_info = NULL, need_compile = need_compile)
#' }
#'
#' #' Compile and load a TMB model
#' #' @noRd
#' compile_and_load <- function(dll_name, cpp_file, cached_path, dirs, is_windows, verbose) {
#'   # Prepare temporary build directory
#'   if (is_windows) {
#'     base_tmp <- "C:/RTMP"
#'     if (!dir.exists(base_tmp)) base_tmp <- tempdir()
#'     build_dir <- file.path(base_tmp, paste0(dll_name, "_", format(Sys.time(), "%Y%m%d%H%M%S")))
#'   } else {
#'     build_dir <- tempfile(pattern = paste0(dll_name, "_build_"))
#'   }
#'   dir.create(build_dir, recursive = TRUE, showWarnings = FALSE)
#'
#'   # Copy source
#'   tmp_cpp <- file.path(build_dir, basename(cpp_file))
#'   file.copy(cpp_file, tmp_cpp, overwrite = TRUE)
#'
#'   # Windows: copy TMB DLLs for linking
#'   if (is_windows) {
#'     dlls <- list.files(system.file("libs", package = "TMB"), pattern = "\\.dll$", full.names = TRUE)
#'     file.copy(dlls, build_dir, overwrite = TRUE)
#'   }
#'
#'   # Compile
#'   old_wd <- getwd()
...
#'   log_msg(verbose, "Compiling TMB model: ", tmp_cpp)
#'   compile_try <- try(
#'     {
#'       TMB::compile(basename(tmp_cpp), safebounds = FALSE, safeunload = FALSE, verbose = verbose)
#'     },
#'     silent = !verbose
#'   )
#'   if (inherits(compile_try, "try-error")) stop("Compilation error: ", conditionMessage(attr(compile_try, "condition")))
#'
#'   # Locate compiled DLL
#'   dll_file <- file.path(build_dir, if (is_windows) paste0(dll_name, ".dll") else TMB::dynlib(dll_name))
#'   if (!file.exists(dll_file)) stop("Compiled DLL not found: ", dll_file)
#'
#'   # Copy to cache and clean up
#'   file.copy(dll_file, cached_path, overwrite = TRUE)
#'   unlink(build_dir, recursive = TRUE, force = is_windows)
#'
#'   # Load into R
#'   dyn.unload(cached_path)
#'   dyn.load(cached_path)
#'   log_msg(verbose, "Compiled and loaded TMB model: ", cached_path)
#'
#'   # Store info
#'   dll_info <- list(
#'     path = cached_path,
...
#'     compiled = TRUE
#'   )
#'   .gkwreg_tmb_env[[paste0("TMB_", dll_name, "_PATH")]] <- dll_info$path
#'   .gkwreg_tmb_env[[paste0("TMB_", dll_name, "_INFO")]] <- dll_info
#'   invisible(dll_info)
#' }
#'
#' #' Unix/macOS implementation for TMB compilation
#' #' @noRd
#' .check_and_compile_TMB_code_unix <- function(dll_name, pkg_name = "gkwreg",
#'                                              force_recompile = FALSE,
#'                                              verbose = FALSE) {
#'   log_msg(verbose, "[Unix] Checking model: ", dll_name)
#'   dirs <- get_cache_dirs(pkg_name, is_windows = FALSE)
#'   dll_ext <- .Platform$dynlib.ext
#'   cached_path <- normalizePath(file.path(dirs$version_dir, paste0(dll_name, dll_ext)), mustWork = FALSE)
#'   cpp_file <- locate_cpp_file(dll_name, pkg_name)
#'   log_msg(verbose, "Source file: ", cpp_file)
#'
#'   load_res <- check_and_load_cached(dll_name, cached_path, cpp_file, force_recompile, verbose)
#'   if (!load_res$need_compile) {
#'     return(invisible(load_res$dll_info))
#'   }
#'
#'   compile_and_load(dll_name, cpp_file, cached_path, dirs, is_windows = FALSE, verbose)
#' }
#'
#' #' Windows-specific implementation for TMB compilation
#' #' @noRd
#' .check_and_compile_TMB_code_windows <- function(dll_name, pkg_name = "gkwreg",
#'                                                 force_recompile = FALSE,
#'                                                 verbose = FALSE) {
#'   log_msg(verbose, "[Windows] Checking model: ", dll_name)
#'   dirs <- get_cache_dirs(pkg_name, is_windows = TRUE)
#'   cached_path <- normalizePath(file.path(dirs$version_dir, paste0(dll_name, ".dll")), mustWork = FALSE)
#'   cpp_file <- locate_cpp_file(dll_name, pkg_name)
#'   log_msg(verbose, "Source file: ", cpp_file)
#'
#'   load_res <- check_and_load_cached(dll_name, cached_path, cpp_file, force_recompile, verbose)
...
#'   }
#'
#'   compile_and_load(dll_name, cpp_file, cached_path, dirs, is_windows = TRUE, verbose)
#' }
#'
#' #' Dispatch based on OS
#' #' @noRd
#' .check_and_compile_TMB_code <- function(dll_name, pkg_name = "gkwreg",
#'                                         force_recompile = FALSE,
#'                                         verbose = FALSE) {
#'   if (.Platform$OS.type == "windows") {
#'     .check_and_compile_TMB_code_windows(dll_name, pkg_name, force_recompile, verbose)
#'   } else {
#'     .check_and_compile_TMB_code_unix(dll_name, pkg_name, force_recompile, verbose)
#'   }
#' }
#'
#' #' Make TMB model available
#' #' @noRd
#' .make_TMB_available <- function(dll_name, silent = FALSE) {
#'   key <- paste0("TMB_", dll_name, "_PATH")
#'   if (exists(key, envir = .gkwreg_tmb_env)) {
#'     path <- .gkwreg_tmb_env[[key]]
#'     if (file.exists(path) && !any(sapply(getLoadedDLLs(), function(x) x$path == path))) dyn.load(path)
#'     return(TRUE)
#'   }
#'   if (!silent) message("Model '", dll_name, "' not loaded; compiling...")
#'   tryCatch(
#'     {
...
#' #' Get TMB DLL info
#' #' @noRd
#' .get_TMB_dll_info <- function(dll_name) {
#'   key <- paste0("TMB_", dll_name, "_INFO")
#'   if (exists(key, envir = .gkwreg_tmb_env)) .gkwreg_tmb_env[[key]] else NULL
#' }
#'
#' #' Package onLoad: precompile known TMB models
#' #' @importFrom utils packageVersion
#' #' @noRd
#' .onLoad <- function(libname, pkgname) {
#'   models <- c("gkwbetareg", "bkwreg", "ekwreg", "gkwmletmb", "gkwreg", "kkwreg", "kwreg", "mcreg")
#'   for (m in models) {
#'     tryCatch(
#'       .check_and_compile_TMB_code(m, pkg_name = pkgname, verbose = FALSE),
#'       error = function(e) warning("Precompile failed for '", m, "': ", e$message)
#'     )
#'   }
#' }
