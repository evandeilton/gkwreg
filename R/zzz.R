...
# Function that detects OS and dispatches to the appropriate implementation
.check_and_compile_TMB_code <- function(dll_name,
  ...
  if (is_windows) {
    return(.check_and_compile_TMB_code_windows(dll_name, pkg_name, force_recompile, verbose))
  } else {
    return(.check_and_compile_TMB_code_unix(dll_name, pkg_name, force_recompile, verbose))
  }
}

# Unix/macOS implementation
.check_and_compile_TMB_code_unix <- function(dll_name,
                                             pkg_name = "gkwreg",
                                             force_recompile = FALSE,
                                             verbose = FALSE) {
  ...
  if (need_compile) {
    ...
    # Copy the cpp file to the temp directory
...
.check_and_compile_TMB_code_windows <- function(dll_name,
                                                pkg_name = "gkwreg",
                                                force_recompile = FALSE,
                                                verbose = FALSE) {
  # Helper function for logging
  ...
  if (verbose) log_msg("Checking TMB model status for ", dll_name, " (Windows)...\n")
  ...
  # 1. Create a persistent cache directory for TMB compilation
  # Use a simple, shorter path for Windows to avoid path length issues
  cache_dir <- file.path("C:/R", paste0(pkg_name, "_tmb_cache"))

  # Fallback to user profile if C:/R is not writable
  if (!dir.exists(cache_dir) && !dir.create(cache_dir, recursive = TRUE, showWarnings = FALSE)) {
    if (requireNamespace("rappdirs", quietly = TRUE)) {
      cache_dir <- file.path(rappdirs::user_cache_dir(pkg_name), "tmb_cache")
    } else {
      # Use Windows-specific AppData folder
  ...
  # 3. Define the full path for the shared object based on platform
  dll_ext <- ".dll" # Windows always uses .dll
  compiled_name <- paste0(dll_name, dll_ext)
  cached_dll_path <- file.path(version_dir, compiled_name)
  ...
  if (file.exists(cached_dll_path) && !force_recompile) {
    ...
    if (file.info(cpp_file)$mtime > file.info(cached_dll_path)$mtime) {
      ...
      if (!inherits(load_result, "try-error")) {
        ...
        # Register the DLL with TMB - critical for Windows CRAN binaries
        try(
          {
            if (requireNamespace("TMB", quietly = TRUE)) {
              TMB::registerDLL(dll_info$name, cached_dll_path)
            }
          },
          silent = TRUE
        )
  ...
  if (need_compile) {
    # Create a simple, shorter temporary path without special characters
    temp_base <- "C:/RTMP"
    if (!dir.exists(temp_base)) {
      if (!dir.create(temp_base, recursive = TRUE, showWarnings = FALSE)) {
        # Fallback to standard temp dir if C:/RTMP can't be created
        temp_base <- tempdir()
      }
    }
    ...
    # Use the base filename only, not full path
    ...
    if (verbose) log_msg("Compiling TMB model from ", temp_cpp_file, " (using basename only)...\n")

    # Windows-specific: copy all TMB-related DLLs to ensure linking works
    ...
    # Windows-specific: Copy RcppEigen include files if needed
    ...
    if (inherits(compile_result, "try-error")) {
      # Enhanced error handling for Windows path issues
      error_msg <- attr(compile_result, "condition")$message
      if (grepl("No such file or directory", error_msg)) {
    ...
    # If first attempt failed, look for alternative filenames
    if (!file.exists(temp_dll_path)) {
    ...
    # On Windows, use unlink with force=TRUE to avoid file locking issues
  ...
  # Register the DLL with TMB - this ensures TMB knows about the models in the DLL
  # This is crucial for Windows when using compiled binaries from CRAN
  try(
    {
      if (requireNamespace("TMB", quietly = TRUE)) {
        TMB::registerDLL(dll_info$name, cached_dll_path)
...
.onLoad <- function(libname, pkgname) {
  ...
  for (model in all_models) {
    tryCatch(
      {
        .check_and_compile_TMB_code(model, pkg_name = pkgname, verbose = FALSE)

        # For Windows, ensure the DLL is properly registered with TMB
        if (is_windows) {
          tmb_info_var <- paste0("TMB_", model, "_INFO")
          tmb_path_var <- paste0("TMB_", model, "_PATH")

          if (exists(tmb_info_var, envir = globalenv()$.gkwreg_env)) {
            dll_info <- get(tmb_info_var, envir = globalenv()$.gkwreg_env)
            if (requireNamespace("TMB", quietly = TRUE)) {
              try(TMB::registerDLL(dll_info$name, dll_info$path), silent = TRUE)
            }
          } else if (exists(tmb_path_var, envir = globalenv()$.gkwreg_env)) {
            dll_path <- get(tmb_path_var, envir = globalenv()$.gkwreg_env)
            dll_name <- basename(tools::file_path_sans_ext(dll_path))
            if (requireNamespace("TMB", quietly = TRUE)) {
              try(TMB::registerDLL(dll_name, dll_path), silent = TRUE)
            }
          }
        }
      },
