...
#' #' Confidence Intervals for Generalized Kumaraswamy Regression Parameters
#' #'
#' #' Computes confidence intervals for model parameters in fitted \code{gkwreg}
#' #' objects. Supports Wald (normal approximation) intervals based on asymptotic
#' #' theory. Follows the same interface as \code{\link[stats]{confint.lm}} and
#' #' \code{\link[stats]{confint.glm}}.
#' #'
#' #' @param object An object of class \code{"gkwreg"} from \code{\link{gkwreg}}.
#' #' @param parm Optional specification of which parameters to compute intervals for.
#' #'   Can be \code{NULL} (default, intervals for all parameters), a character vector
#' #'   of parameter names or patterns, or a numeric vector of parameter indices.
#' #' @param level Numeric. The confidence level required. Must be between 0 and 1.
#' #'   Default is 0.95 (95\% confidence intervals).
#' #' @param ... Additional arguments (currently unused).
#' #'
#' #' @details
#' #' \strong{Wald Intervals:}
#' #'
#' #' The confidence intervals are computed using the Wald method based on
#' #' asymptotic normality of maximum likelihood estimators:
#' #' \deqn{CI = \hat{\theta} \pm z_{\alpha/2} \times SE(\hat{\theta})}
...
#' #' These intervals are:
#' #' \itemize{
#' #'   \item Fast to compute (no refitting required)
#' #'   \item Appropriate for large samples
#' #'   \item Valid when likelihood is approximately normal
#' #' }
#' #'
#' #' \strong{Requirements:}
#' #'
#' #' The model must have been fitted with \code{hessian = TRUE} (the default)
#' #' in \code{\link{gkw_control}}. If standard errors are not available, an
#' #' error is raised with instructions to refit the model.
#' #'
#' #' \strong{Parameter Selection:}
#' #'
#' #' The \code{parm} argument allows flexible parameter selection:
#' #' \itemize{
#' #'   \item \code{NULL}: All parameters (default)
#' #'   \item Character vector: Exact parameter names (e.g., \code{"alpha:(Intercept)"})
#' #'   \item Numeric vector: Parameter indices (e.g., \code{1:3})
#' #' }
#' #'
#' #' \strong{Flexibility:}
#' #'
#' #' Unlike the deprecated \code{conf.level} argument in older versions of
#' #' \code{gkwreg()}, this function can compute intervals at any confidence
#' #' level without refitting the model. Multiple confidence levels can be
#' #' obtained efficiently.
#' #'
#' #' @return A matrix with columns giving lower and upper confidence limits
#' #'   for each parameter. Row names correspond to parameter names. Column names
#' #'   indicate the confidence level (e.g., "2.5 \%", "97.5 \%").
#' #'
#' #' @examples
#' #' \donttest{
#' #' # Fit model
...
#' #' fit <- gkwreg(yield ~ batch + temp, data = GasolineYield, family = "kw")
#' #'
#' #' # Default: 95\% intervals for all parameters
#' #' confint(fit)
#' #'
...
#' #' confint(fit, parm = "alpha:(Intercept)")
#' #' confint(fit, parm = c("alpha:temp", "beta:temp"))
#' #'
#' #' # Using numeric indices
#' #' confint(fit, parm = 1:3)
#' #'
#' #' # Multiple confidence levels (no refitting needed!)
...
#' #'
#' #' @seealso
#' #' \code{\link{gkwreg}} for model fitting,
...
#' #' @method confint gkwreg
#' #' @export
#' confint.gkwreg <- function(object,
#'                            parm = NULL,
#'                            level = 0.95,
#'                            ...) {
#'   # INPUT VALIDATION
...
#'   }
#'
#'   # Check if model has standard errors
#'   if (is.null(object$vcov) || is.null(object$se)) {
#'     stop(
#'       "Cannot compute confidence intervals: standard errors not available.\n",
#'       "  Reason: Model was fitted with hessian = FALSE.\n",
#'       "  Solution: Refit with control = gkw_control(hessian = TRUE)",
#'       call. = FALSE
#'     )
#'   }
#'
#'   # Check for NA standard errors (Hessian computation failed)
#'   if (all(is.na(object$se))) {
#'     stop(
#'       "Cannot compute confidence intervals: standard error computation failed.\n",
#'       "  This may indicate numerical problems during fitting.\n",
#'       "  Try: (1) different starting values, (2) different optimizer method,\n",
#'       "       (3) rescaling predictors, or (4) simpler model specification.",
#'       call. = FALSE
#'     )
#'   }
#'
...
#'   n_params <- length(cf)
#'
#'   # Determine which parameters to compute intervals for
#'   if (is.null(parm)) {
#'     # All parameters
#'     parm_idx <- seq_len(n_params)
#'   } else if (is.numeric(parm)) {
#'     # Numeric indices
#'     if (any(parm < 1) || any(parm > n_params)) {
#'       stop(
#'         "'parm' indices must be between 1 and ", n_params,
#'         call. = FALSE
#'       )
#'     }
#'     parm_idx <- as.integer(parm)
#'   } else if (is.character(parm)) {
#'     # Character names - allow partial matching
#'     parm_idx <- integer(0)
#'
#'     for (p in parm) {
#'       # Try exact match first
#'       exact_match <- which(all_names == p)
#'
#'       if (length(exact_match) > 0) {
#'         parm_idx <- c(parm_idx, exact_match)
#'       } else {
#'         # Try partial match
#'         partial_match <- grep(p, all_names, fixed = TRUE)
#'
#'         if (length(partial_match) == 0) {
#'           warning(
#'             "Parameter '", p, "' not found. Skipping.",
#'             call. = FALSE, immediate. = TRUE
#'           )
#'         } else if (length(partial_match) > 1) {
#'           warning(
#'             "Parameter pattern '", p, "' matches multiple parameters: ",
#'             paste(all_names[partial_match], collapse = ", "),
#'             ". Using all matches.",
#'             call. = FALSE, immediate. = TRUE
#'           )
#'           parm_idx <- c(parm_idx, partial_match)
#'         } else {
#'           parm_idx <- c(parm_idx, partial_match)
#'         }
#'       }
...
#'     parm_idx <- sort(unique(parm_idx))
#'
#'     if (length(parm_idx) == 0) {
#'       stop("No valid parameters found matching 'parm'", call. = FALSE)
#'     }
#'   } else {
#'     stop(
#'       "'parm' must be NULL, numeric indices, or character names",
#'       call. = FALSE
#'     )
#'   }
#'
...
#'
#'
#'   # Extract coefficients and SEs for selected parameters
#'   cf_selected <- cf[parm_idx]
#'   se_selected <- object$se[parm_idx]
#'
#'   # Check for NA standard errors in selected parameters
#'   if (any(is.na(se_selected))) {
#'     na_params <- all_names[parm_idx][is.na(se_selected)]
#'     warning(
#'       "Standard errors not available for: ",
#'       paste(na_params, collapse = ", "), "\n",
#'       "  Confidence intervals for these parameters will be NA.",
#'       call. = FALSE
#'     )
#'   }
#'
#'   # Compute z-score for the specified confidence level
#'   alpha <- 1 - level
#'   z_score <- stats::qnorm(1 - alpha / 2)
#'
#'   # Compute confidence intervals
#'   ci_lower <- cf_selected - z_score * se_selected
#'   ci_upper <- cf_selected + z_score * se_selected
#'
#'   # Construct result matrix
#'   ci_matrix <- cbind(Lower = ci_lower, Upper = ci_upper)
#'   rownames(ci_matrix) <- names(cf_selected)
#'
#'   # Format column names to show confidence level
#'   lower_pct <- format((alpha / 2) * 100, digits = 3, trim = TRUE)
#'   upper_pct <- format((1 - alpha / 2) * 100, digits = 3, trim = TRUE)
#'   colnames(ci_matrix) <- paste0(c(lower_pct, upper_pct), " %")
#'
#'   return(ci_matrix)
#' }
