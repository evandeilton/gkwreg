...
# SEED STRATEGY:
# - Global seed (2024) for reproducibility
# - Local seeds (123, 456, etc.) used for specific data visualizations
# - The Monte Carlo study is precomputed and seeded in
...
# The Monte Carlo study below (3 scenarios x 200 replications x 4 models) is
# precomputed. Running it live made the vignette take about 11 minutes to
# rebuild on the CRAN check machines, which is more than CRAN can afford to run
# regularly. The study itself is unchanged -- still 200 replications -- but the
# summaries are read from a stored object instead of being recomputed here.
#
# Regenerate with: Rscript data-raw/vignette-simulations.R
sim_results <- readRDS(
  system.file("extdata", "vignette-simulations.rds", package = "gkwreg")
)

# Quantities quoted in the prose below. They are derived from the tables rather
# than written out by hand, so the narrative cannot drift away from the results
# when the study is regenerated.
.pick <- function(tab, model, col) tab[[col]][tab$Model == model]
.speedup <- function(tab) {
  .pick(tab, "Beta (betareg)", "Time") / .pick(tab, "Kumaraswamy", "Time")
}

speedup_s1 <- .speedup(sim_results$comp_s1)
speedup_s2 <- .speedup(sim_results$comp_s2)
speedup_s3 <- .speedup(sim_results$comp_s3)
speedup_lo <- min(speedup_s1, speedup_s2, speedup_s3)
speedup_hi <- max(speedup_s1, speedup_s2, speedup_s3)

conv_beta_s3 <- .pick(sim_results$comp_s3, "Beta (betareg)", "Conv_Rate")
conv_kw_s3 <- .pick(sim_results$comp_s3, "Kumaraswamy", "Conv_Rate")
n_conv_beta_s3 <- round(conv_beta_s3 / 100 * sim_results$n_sim)

aic_beta_s1 <- .pick(sim_results$comp_s1, "Beta (betareg)", "AIC")
aic_kw_s1 <- .pick(sim_results$comp_s1, "Kumaraswamy", "AIC")
...
Through Monte Carlo simulations across three distinct data-generating mechanisms—well-specified Beta data, heavy-tailed distributions, and extreme boundary-concentrated patterns—we demonstrate that Kumaraswamy-based models provide substantial computational advantages (`r fmt_x(speedup_lo)`-`r fmt_x(speedup_hi)`× faster) with equivalent or superior statistical performance. Parameter estimation analysis reveals that coefficient estimates remain consistent across models, but standard errors can differ by 15-40% depending on distributional misspecification. Notably, for non-standard distributional shapes, Kumaraswamy models achieve dramatically higher convergence rates (`r conv_kw_s3`% vs. `r conv_beta_s3`% for extreme U-shaped data) and improved model fit metrics.
...
Table 3 documents Beta regression's **catastrophic failure** when confronted with extreme shapes. Among `r sim_results$n_sim` simulation replicates, Beta regression converged only `r n_conv_beta_s3` times (`r conv_beta_s3`%), while Kumaraswamy achieved perfect reliability (`r conv_kw_s3`%).

```{r scenario3_coef_comparison}
...
**When Beta does converge** (the rare `r conv_beta_s3`% of cases), the SE Ratio analysis reveals instability:
...
knitr::kable(
  all_scenarios,
  row.names = FALSE,
  caption = "Table 4: Comprehensive Model Comparison Across Three Simulation Scenarios. Note: Scenario 3 Beta statistics are based on only `r conv_beta_s3`% successful fits; metrics may not be representative.",
...
1. **Scenario 1 (Well-specified)**: Beta achieves marginally better AIC (`r round(aic_beta_s1)` vs `r round(aic_kw_s1)`), but Kumaraswamy is `r fmt_x(speedup_s1)`× faster, with both converging in every replicate
...
3. **Scenario 3 (Extreme shapes)**: Beta's AIC becomes uninformative due to convergence failure; Kumaraswamy provides the sole viable solution, with a `r fmt_x(speedup_s3)`× speedup (note that betareg's mean time counts its many futile optimisations)
...
The speedup for Kumaraswamy ranges from `r fmt_x(speedup_lo)`× to `r fmt_x(speedup_hi)`× and depends far more on whether Beta regression is *able* to fit the data than on model complexity:

- Scenarios 1 and 2, where both families converge everywhere: `r fmt_x(speedup_s1)`× and `r fmt_x(speedup_s2)`× -- a modest edge from the closed-form CDF.
- Scenario 3, where Beta converges in only `r conv_beta_s3`% of replicates: `r fmt_x(speedup_s3)`×. Most of that gap is Beta's optimiser iterating at length before failing, so it measures the cost of misspecification rather than a like-for-like speed comparison.

Timings come from the precomputed run recorded in the object above (`r sim_results$r_version`, `r format(sim_results$generated_on)`); absolute values are machine-dependent, the ratios much less so.
...
1. **Computational efficiency**: `r fmt_x(speedup_lo)`-`r fmt_x(speedup_hi)`× faster estimation, with the large end of that range driven by Beta regression's failure to converge rather than by raw speed

2. **Numerical stability**: `r conv_kw_s3`% vs. `r conv_beta_s3`% convergence success for extreme distributions, representing a **qualitative reliability difference**
...
3. **Computational cost of misspecification**: Quantified how Beta regression's advantage disappears once its assumptions fail, from a `r fmt_x(speedup_s1)`× difference under a well-specified Beta to `r fmt_x(speedup_s3)`× when convergence breaks down
