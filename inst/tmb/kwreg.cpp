...
// 
// // ========================
// // Kw log-PDF - Simple implementation for Kumaraswamy
// // ========================
// template <class Type>
// Type log_pdf_kw(const Type &y,
//                 const Type &alpha,
//                 const Type &beta) {
...
//   Type term3 = (beta - Type(1.0)) * safeLog(one_minus_ya);
// 
//   // Combine all terms to get log-likelihood
//   Type logf = term1 + term2 + term3;
// 
...
// 
// // ========================
// // Calculate CDF of Kw distribution
// // ========================
// template <class Type>
// Type cdf_kw(const Type &y,
//             const Type &alpha,
//             const Type &beta) {
...
// 
//   // Step 4: 1-(1-y^α)^β - Final CDF value
//   return Type(1.0) - one_minus_ya_beta;
// }
// 
// // ========================
...
// // ========================
// template <class Type>
// Type calc_mean_kw(const Type &alpha,
//                   const Type &beta) {
//   const int n_points = 30;
// 
...
// 
// // ========================
// // Analytical mean for Kumaraswamy
// // ========================
// template <class Type>
// Type exact_mean_kw(const Type &alpha, const Type &beta) {
//   // E(X) = β * B(1 + 1/α, β)
//   // For Kumaraswamy, E(X) = β * B(1 + 1/α, β) / B(1, β)
//   // Simplified to E(X) = β/(β+1) * Γ(1+1/α) * Γ(β) / Γ(1+1/α+β)
//   // Further simplified using beta function properties to:
//   // E(X) = β * B(1+1/α, β)
// 
//   // Define term1 = 1 + 1/α
//   Type term1 = Type(1.0) + Type(1.0) / alpha;
// 
//   // Calculate log(B(1+1/α, β)) using lgamma
//   Type log_beta_func = lgamma(term1) + lgamma(beta) - lgamma(term1 + beta);
// 
//   // Return β * B(1+1/α, β)
//   return beta * exp(log_beta_func);
...
//     if (calcFitted == 1) {
//       // For Kumaraswamy, we can use the exact formula for the mean
//       fitted(i) = exact_mean_kw(alpha_i, beta_i);
// 
//       // Alternatively, we could use numerical integration with caching:
//       /*
//        // Check if we should use caching (useMeanCache==1)
//        if (useMeanCache == 1) {
//        std::vector<int> key = make_cache_key(alpha_i, beta_i);
// 
//        auto it = meanCache.find(key);
//        if (it != meanCache.end()) {
//        fitted(i) = Type(it->second);
//        } else {
//        Type mval = calc_mean_kw(alpha_i, beta_i);
//        meanCache[key] = asDouble(mval);
//        fitted(i) = mval;
//        }
//        } else {
//        // No cache, calculate directly
//        fitted(i) = calc_mean_kw(alpha_i, beta_i);
//        }
//        */
//     }
//   }
...
//   vector<Type> alphaVec(n), betaVec(n);
// 
