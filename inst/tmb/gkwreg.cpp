...
// // ========================
// template <class Type>
// inline Type log_pdf_gkw(const Type &y, const Type &alpha, const Type &beta, 
//                         const Type &gamma, const Type &delta, const Type &lambda) {
//   // Quick validation using asDouble (fast in double mode)
//   double y_d = asDouble(y);
//   if (y_d <= 1e-12 || y_d >= 0.999999) return -Constants<Type>::inf_repl;
//   
...
//   double lambda_d = asDouble(lambda);
//   
//   if (alpha_d <= 1e-10 || beta_d <= 1e-10 || gamma_d <= 1e-10 || 
//       delta_d <= 1e-10 || lambda_d <= 1e-10) {
//     return -Constants<Type>::inf_repl;
//   }
//   
...
//   
//   // Combine all terms:
//   // log f = log(λαβ) - log[B(γ, δ+1)] + (α-1)log(y) + (β-1)log(1-y^α)
//   //         + (γλ-1)log[v] + δlog[u]
//   
//   Type term1 = log_lambda + log_alpha + log_beta - logB_val;
//   Type term2 = (alpha - Type(1.0)) * log(y);
...
// // ========================
// template <class Type>
// inline Type calc_mean_gkw(const Type &alpha, const Type &beta, const Type &gamma,
