...
test_that("gkwfit handles extreme parameter values", {
  ...
  expect_equal(fit$convergence, FALSE)
...
test_that("Fitted parameters follow expected relationships in nested models", {
  ...
  expect_lt(abs(fit_bkw$coefficients["gamma"] - 1), 1)
  expect_lt(abs(fit_bkw$coefficients["delta"]), 2)

  # Kw is GKw with gamma = 1, delta = 0, lambda = 1
  expect_lt(abs(fit_gkw$coefficients["gamma"] - 1), 2)
  expect_lt(abs(fit_gkw$coefficients["delta"]), 2)
  expect_lt(abs(fit_gkw$coefficients["lambda"] - 1), 2)

  # Check that likelihoods follow expected ordering (more general models have higher LL)
  ll_kw <- logLik(fit_kw)[1]
  ll_ekw <- logLik(fit_ekw)[1]
  ll_bkw <- logLik(fit_bkw)[1]
  ll_gkw <- logLik(fit_gkw)[1]

  expect_true(ll_gkw >= ll_bkw && ll_gkw >= ll_ekw && ll_gkw >= ll_kw)
})
