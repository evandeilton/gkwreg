...
library(testthat)
context("Distribution Fitting Functions")
...
test_that("gkwfit works correctly for Generalized Kumaraswamy distribution", {
  ...
  expect_lt(abs(fit_gkw$coefficients["alpha"] - alpha), 2.0)
  expect_lt(abs(fit_gkw$coefficients["beta"] - beta), 1.0)
  expect_lt(abs(fit_gkw$coefficients["gamma"] - gamma), 1.0)
  expect_lt(abs(fit_gkw$coefficients["delta"] - delta), 2.0)
  expect_lt(abs(fit_gkw$coefficients["lambda"] - lambda), 2.0)
})
...
test_that("compare_gof_plot works correctly", {
  ...
  plot_list <- compare_gof_plot(list(KW = gof_kw, Beta = gof_beta), plot_type = "all")

  # Basic check on the output
